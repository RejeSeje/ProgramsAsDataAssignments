...
let rec cStmt stmt (varEnv : varEnv) (funEnv : funEnv) : instr list = 
    ...
    | Switch (e, caselist) ->
      build func der laver


      let controlExprL = cExpr e varEnv funEnv
      let controlExpr = List.head controlExprL
      let rec loop cases controlExpr = 
        match caselist with
        | [] -> 0
        | (i, stmt) :: xs ->
          if (controlExpr = i) then cStmt stmt varEnv funEnv else loop xs
      loop caselist controlExpr
    | Return None -> 
      [RET (snd varEnv - 1)]
    | Return (Some e) -> 
