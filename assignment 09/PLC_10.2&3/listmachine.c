...
void markPhase(word s[], word sp) {
  word i;
  // For-loop scanning the abstract machine stack
  for (i = 0; i <= sp; i++) {
    // Check if heap reference in stack is non nill
    if (s[i] != 0 && !IsInt(s[i])) {
      // Mark the block
      word* block = (word*)s[i];
      mark (block);      
...
void mark(word* block) {
  // Paint the block black, because it is live and before sweepphase
  Paint(block, Black);
}

void sweepPhase() {
    word* currentBlock = heap;

  while (currentBlock < afterHeap) {
    unsigned int color = Color(currentBlock[0]);

    if (color == Black) {
      // Paint black blocks white
      Paint(currentBlock[0], White);
    } else if (color == White) {
      //PLC 10.3
      // Checks if next block is white as well
      word* nextBlock = currentBlock + Length(currentBlock[0]) + 1;

      if (Color(nextBlock) == White) {
        // If so, combine and add to freelist as one block
        currentBlock[1] = (word)freelist;
        freelist = currentBlock + nextBlock;
      } else {
        // If not, simply add the original block to freelist
        currentBlock[1] = (word)freelist;
        freelist = currentBlock;
      }
    }
    currentBlock += Length(currentBlock[0]) + 1;
  }
}
