...
let rec typ (e : tyexpr) (env : typ env) : typ =
    ...
    | ListExpr(typList, givenTyp) ->
      match typList with
      | [] -> TypL(givenTyp)
      | x :: xs ->
        let xTyp = typ x env
        if xTyp = givenTyp then typ (ListExpr(xs, givenTyp)) env else failwith "ListExpr: not monomorphic"
...
let exErr4 = Letfun("f", "x", TypB, If(Var "x", CstI 11, CstI 22), TypB,
                    Call(Var "f", CstB true));;
let listE = ListExpr([CstI 17; CstI 18], TypI);;
let listEE = ListExpr([CstB false; CstI 17], TypI)